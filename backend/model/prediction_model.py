...
def preprocess_data(data: pd.DataFrame, target_col: str) -> pd.DataFrame:  # Fixed: added target_col parameter type
    """
    ...
    if len(categorical_cols) > 0:  # Fixed: changed from categorical_cols.any() to len(categorical_cols) > 0
...
def train_random_forest(X_train: pd.DataFrame, y_train: pd.Series, n_estimators: int = 100, random_state: int = 42):  # Fixed: changed y_train type from DataFrame to Series
    """
...
def train_xgboost(X_train: pd.DataFrame, y_train: pd.Series, n_estimators: int = 100, random_state: int = 42):  # Fixed: changed y_train type from DataFrame to Series
    ...
    # Fixed: Align indices to prevent length mismatch
...
def evaluate_model(model, X_test: pd.DataFrame, y_test: pd.Series) -> dict:  # Fixed: changed y_test type from DataFrame to Series
    """
    Evaluate the trained model on the test set.
    """
    from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score
    
    # Fixed: Handle XGBoost model evaluation
    if hasattr(model, 'get_booster'):  # XGBoost check
